---
title: Accessing the PlaidCloud API from a Pod
content_type: task
weight: 120
---
...
This guide demonstrates how to access the PlaidCloud API from within a pod.
...
The easiest way to use the PlaidCloud API from a Pod is to use
...
From within a Pod, the recommended ways to connect to the PlaidCloud API are:

  - For a Go client, use the official [Go client library](https://github.com/PlaidCloud/client-go/).
  ...
  - For a Python client, use the official [Python client library](https://github.com/PlaidCloud-client/python/).
    The `config.load_incluster_config()` function handles API host discovery and authentication automatically.
    See [an example here](https://github.com/PlaidCloud-client/python/blob/master/examples/in_cluster_config.py).
...
While running in a Pod, the PlaidCloud apiserver is accessible via a Service named
`PlaidCloud` in the `default` namespace. Therefore, Pods can use the
`PlaidCloud.default.svc` hostname to query the API server. Official client libraries
...
APISERVER=https://PlaidCloud.default.svc

# Path to ServiceAccount token
SERVICEACCOUNT=/var/run/secrets/PlaidCloud.io/serviceaccount
