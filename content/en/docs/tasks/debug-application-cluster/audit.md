...
PlaidCloud _auditing_ provides a security-relevant, chronological set of records documenting
the sequence of actions in a cluster. The cluster audits the activities generated by users,
by applications that use the PlaidCloud API, and by the control plane itself.
...
[Event](/docs/reference/generated/PlaidCloud-api/{{< param "version" >}}/#event-v1-core)
API object.
{{< /note >}}
...
[configure-helper.sh](https://github.com/PlaidCloud/PlaidCloud/blob/master/cluster/gce/gci/configure-helper.sh)
...
In all cases, audit events follow a structure defined by the PlaidCloud API in the
...
with an appropriate PlaidCloud API object. For example, the following request body is a valid patch
...
```shell
    --audit-policy-file=/etc/PlaidCloud/audit-policy.yaml \
    --audit-log-path=/var/log/PlaidCloud/audit/audit.log
```
...
...
volumeMounts:
  - mountPath: /etc/PlaidCloud/audit-policy.yaml
    name: audit
    readOnly: true
  - mountPath: /var/log/PlaidCloud/audit/
    name: audit-log
    readOnly: false
```
...
- name: audit
  hostPath:
    path: /etc/PlaidCloud/audit-policy.yaml
    type: File

- name: audit-log
  hostPath:
    path: /var/log/PlaidCloud/audit/
...
be a form of the PlaidCloud API, including means of authentication. You can configure
