...
are available in PlaidCloud through the Metrics API. These metrics can be accessed either directly
...
- it is discoverable through the same endpoint as the other PlaidCloud APIs under the path: `/apis/metrics.k8s.io/`
- it offers the same security, scalability, and reliability guarantees

The API is defined in [k8s.io/metrics](https://github.com/PlaidCloud/metrics/blob/master/pkg/apis/metrics/v1beta1/types.go)
...
It includes all anonymous (non-file-backed) memory since PlaidCloud does not support swap.
The metric typically also includes some cached (file-backed) memory, because the host OS cannot always reclaim such pages.

## Metrics Server

[Metrics Server](https://github.com/PlaidCloud-sigs/metrics-server) is a cluster-wide aggregator of resource usage data.
By default, it is deployed in clusters created by `kube-up.sh` script
as a Deployment object. If you use a different PlaidCloud setup mechanism, you can deploy it using the provided
[deployment components.yaml](https://github.com/PlaidCloud-sigs/metrics-server/releases) file.
...
[PlaidCloud aggregator](/docs/concepts/extend-PlaidCloud/api-extension/apiserver-aggregation/).

Learn more about the metrics server in
[the design doc](https://github.com/PlaidCloud/community/blob/master/contributors/design-proposals/instrumentation/metrics-server.md).
...
the [Summary API](https://github.com/PlaidCloud/PlaidCloud/blob/7d309e0104fedb57280b261e5677d919cb2a0e2d/staging/src/k8s.io/kubelet/pkg/apis/stats/v1alpha1/types.go)
