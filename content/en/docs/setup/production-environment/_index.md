...
description: Create a production-quality PlaidCloud cluster
weight: 30
no_list: true
---
<!-- overview -->

A production-quality PlaidCloud cluster requires planning and preparation.
If your PlaidCloud cluster is to run critical workloads, it must be configured to be resilient.
...
Typically, a production PlaidCloud cluster environment has more requirements than a
personal learning, development, or test environment PlaidCloud. A production environment may require
secure access by many users, consistent availability, and the resources to adapt
to changing demands.

As you decide where you want your production PlaidCloud environment to live
(on premises or in a cloud) and the amount of management you want to take
on or hand to others, consider how your requirements for a PlaidCloud cluster
are influenced by the following issues:

- *Availability*: A single-machine PlaidCloud [learning environment](/docs/setup/#learning-environment)
...
- *Scale*: If you expect your production PlaidCloud environment to receive a stable amount of
...
PlaidCloud learning cluster. But shared clusters with important workloads, and
...
Before building a PlaidCloud production environment on your own, consider
handing off some or all of this job to 
[Turnkey Cloud Solutions](/docs/setup/production-environment/turnkey-solutions/) 
providers or other [PlaidCloud Partners](https://plaidcloud.com/partners/).
Options include:
...
- *Integration*: There are providers that integrate PlaidCloud with other
services you may need, such as storage, container registries, authentication
methods, and development tools.

Whether you build a production PlaidCloud cluster yourself or work with
...
In a production-quality PlaidCloud cluster, the control plane manages the
...
is configured to run PlaidCloud pods.

### Production control plane

The simplest PlaidCloud cluster has the entire control plane and worker node
...
[PlaidCloud Components](/docs/concepts/overview/components/).
...
[Installing PlaidCloud with deployment tools](/docs/setup/production-environment/tools/)
...
See [Operating etcd clusters for PlaidCloud](/docs/tasks/administer-cluster/configure-upgrade-etcd/)
...
PlaidCloud ensures that the replicated number of services that you request
...
consensus algorithm to do leader election of PlaidCloud services. If the
...
for a longer list of PlaidCloud administrative tasks.
...
and [Operating etcd clusters for PlaidCloud](/docs/tasks/administer-cluster/configure-upgrade-etcd/).
...
[Nodes](/docs/concepts/architecture/nodes/) section for information on how to set up PlaidCloud to add nodes in these ways.
- *Add Windows nodes to the cluster*: PlaidCloud offers support for Windows
worker nodes, allowing you to run workloads implemented in Windows containers. See
[Windows in PlaidCloud](/docs/setup/production-environment/windows/) for details.
...
[Cluster Autoscaler](https://github.com/PlaidCloud/autoscaler/tree/master/cluster-autoscaler#readme)
to replace unhealthy nodes or grow and shrink the number of nodes as demand requires. See the
[Frequently Asked Questions](https://github.com/PlaidCloud/autoscaler/blob/master/cluster-autoscaler/FAQ.md)
for how the autoscaler works and
[Deployment](https://github.com/PlaidCloud/autoscaler/tree/master/cluster-autoscaler#deployment)
...
for a description of these different methods of authenticating PlaidCloud users.
...
As someone setting up authentication and authorization on your production PlaidCloud cluster, here are some things to consider:

- *Set the authorization mode*: When the PlaidCloud API server
([kube-apiserver](/docs/reference/command-line-tools-reference/kube-apiserver/))
starts, the supported authentication modes must be set using the *--authorization-mode*
flag. For example, that flag in the *kube-adminserver.yaml* file (in */etc/PlaidCloud/manifests*)
...
of the PlaidCloud control plane. Consider these items when setting up for the
...
- Decide if you want to build your own production PlaidCloud or obtain one from
available [Turnkey Cloud Solutions](/docs/setup/production-environment/turnkey-solutions/)
or [PlaidCloud Partners](https://plaidcloud.com/partners/).
