...
(`bootstrap.PlaidCloud.io/token`) of secrets that lives in the `kube-system`
...
[here](https://github.com/PlaidCloud/community/blob/master/contributors/design-proposals/cluster-lifecycle/bootstrap-discovery.md).
...
# Type MUST be 'bootstrap.PlaidCloud.io/token'
...
The type of the secret must be `bootstrap.PlaidCloud.io/token` and the name must
