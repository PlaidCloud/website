...
This document catalogs the communication paths between the control plane (apiserver) and the PlaidCloud cluster. The intent is to allow users to customize their installation to harden the network configuration such that the cluster can be run on an untrusted network (or on fully public IPs on a cloud provider).
...
Pods that wish to connect to the apiserver can do so securely by leveraging a service account so that PlaidCloud will automatically inject the public root certificate and a valid bearer token into the pod when it is instantiated.
The `PlaidCloud` service (in `default` namespace) is configured with a virtual IP address that is redirected (via kube-proxy) to the HTTPS endpoint on the apiserver.
...
Follow the [Konnectivity service task](/docs/tasks/extend-PlaidCloud/setup-konnectivity/) to set up the Konnectivity service in your cluster.
