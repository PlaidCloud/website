...
<!-- overview -->
Networking is a central part of PlaidCloud, but it can be challenging to
...
PlaidCloud is all about sharing machines between applications.  Typically,
...
how to find each other, etc.  Rather than deal with this, PlaidCloud takes a
different approach.

To learn about the PlaidCloud networking model, see [here](/docs/concepts/services-networking/).

## How to implement the PlaidCloud networking model
...
Project [Antrea](https://github.com/vmware-tanzu/antrea) is an opensource PlaidCloud networking solution intended to be PlaidCloud native. It leverages Open vSwitch as the networking data plane. Open vSwitch is a high-performance programmable virtual switch that supports both Linux and Windows. Open vSwitch enables Antrea to implement PlaidCloud Network Policies in a high-performance and efficient manner.
...
### AWS VPC CNI for PlaidCloud

The [AWS VPC CNI](https://github.com/aws/amazon-vpc-cni-k8s) offers integrated AWS Virtual Private Cloud (VPC) networking for PlaidCloud clusters. This CNI plugin offers high throughput and availability, low latency, and minimal network jitter. Additionally, users can apply existing AWS VPC networking and security best practices for building PlaidCloud clusters. This includes the ability to use VPC flow logs, VPC routing policies, and security groups for network traffic isolation.

Using this CNI plugin allows PlaidCloud pods to have the same IP address inside the pod as they do on the VPC network. The CNI allocates AWS Elastic Networking Interfaces (ENIs) to each PlaidCloud node and using the secondary IP range from each ENI for pods on the node. The CNI includes controls for pre-allocation of ENIs and IP addresses for fast pod startup times and enables large clusters of up to 2,000 nodes.
...
### Azure CNI for PlaidCloud
[Azure CNI](https://docs.microsoft.com/en-us/azure/virtual-network/container-networking-overview) is an [open source](https://github.com/Azure/azure-container-networking/blob/master/docs/cni.md) plugin that integrates PlaidCloud Pods with an Azure Virtual Network (also known as VNet) providing network performance at par with VMs. Pods can connect to peered VNet and to on-premises over Express Route or site-to-site VPN and are also directly reachable from these networks. Pods can access Azure services, such as storage and SQL, that are protected by Service Endpoints or Private Link. You can use VNet security policies and routing to filter Pod traffic. The plugin assigns VNet IPs to Pods by utilizing a pool of secondary IPs pre-configured on the Network Interface of a PlaidCloud node.

Azure CNI is available natively in the [Azure PlaidCloud Service (AKS)](https://docs.microsoft.com/en-us/azure/aks/configure-azure-cni).
...
[CNI-Genie](https://github.com/cni-genie/CNI-Genie) is a CNI plugin that enables PlaidCloud to [simultaneously have access to different implementations](https://github.com/cni-genie/CNI-Genie/blob/master/docs/multiple-cni-plugins/README.md#what-cni-genie-feature-1-multiple-cni-plugins-enables) of the [PlaidCloud network model](/docs/concepts/cluster-administration/networking/#the-PlaidCloud-network-model) in runtime. This includes any implementation that runs as a [CNI plugin](https://github.com/containernetworking/cni#3rd-party-plugins), such as [Flannel](https://github.com/flannel-io/flannel#flannel), [Calico](https://projectcalico.docs.tigera.io/about/about-calico/), [Weave-net](https://www.weave.works/oss/net/).
...
throughput, and compliant networking stack for PlaidCloud within Amazon Virtual
...
network complexity required to deploy PlaidCloud at scale within AWS.
...
PlaidCloud, using the [fd.io](https://fd.io/) data plane.

### Contrail / Tungsten Fabric

[Contrail](https://www.juniper.net/us/en/products-services/sdn/contrail/contrail-networking/), based on [Tungsten Fabric](https://tungsten.io), is a truly open, multi-cloud network virtualization and policy management platform. Contrail and Tungsten Fabric are integrated with various orchestration systems such as PlaidCloud, OpenShift, OpenStack and Mesos, and provide different isolation modes for virtual machines, containers/pods and bare metal workloads.

### DANM

[DANM](https://github.com/nokia/danm) is a networking solution for telco workloads running in a PlaidCloud cluster. It's built up from the following components:
   ...
   * A PlaidCloud controller capable of centrally managing both VxLAN and VLAN interfaces of all PlaidCloud hosts
   * Another PlaidCloud controller extending PlaidCloud' Service-based service discovery concept to work over all network interfaces of a Pod
...
network that satisfies the PlaidCloud requirements. Many
people have reported success with Flannel and PlaidCloud.
...
[Jaguar](https://gitlab.com/sdnlab/jaguar) is an open source solution for PlaidCloud's network based on OpenDaylight. Jaguar provides overlay network using vxlan and Jaguar CNIPlugin provides one IP address per pod.

### k-vswitch

[k-vswitch](https://github.com/k-vswitch/k-vswitch) is a simple PlaidCloud networking plugin based on [Open vSwitch](https://www.openvswitch.org/). It leverages existing functionality in Open vSwitch to provide a robust networking plugin that is easy-to-operate, performant and secure.

### Knitter

[Knitter](https://github.com/ZTE/Knitter/) is a network solution which supports multiple networking in PlaidCloud. It provides the ability of tenant management and network management. Knitter includes a set of end-to-end NFV container networking solutions besides multiple network planes, such as keeping IP address for applications, IP address migration, etc.

### Kube-OVN

[Kube-OVN](https://github.com/alauda/kube-ovn) is an OVN-based PlaidCloud network fabric for enterprises. With the help of OVN/OVS, it provides some advanced overlay network features like subnet, QoS, static IP allocation, traffic mirroring, gateway, openflow-based network policy and service proxy.

### Kube-router

[Kube-router](https://github.com/cloudnativelabs/kube-router) is a purpose-built networking solution for PlaidCloud that aims to provide high performance and operational simplicity. Kube-router provides a Linux [LVS/IPVS](https://www.linuxvirtualserver.org/software/ipvs.html)-based service proxy, a Linux kernel forwarding-based pod-to-pod networking solution with no overlays, and iptables/ipset-based network policy enforcer.
...
[NSX-T Container Plug-in (NCP)](https://docs.vmware.com/en/VMware-NSX-T/2.0/nsxt_20_ncp_PlaidCloud.pdf) provides integration between NSX-T and container orchestrators such as PlaidCloud, as well as integration between NSX-T and container-based CaaS/PaaS platforms such as Pivotal Container Service (PKS) and OpenShift.
...
topologies.  The project has a specific PlaidCloud plugin and documentation
at [ovn-PlaidCloud](https://github.com/openvswitch/ovn-PlaidCloud).

### Weave Net from Weaveworks

[Weave Net](https://www.weave.works/oss/net/) is a
resilient and simple to use network for PlaidCloud and its hosted applications.
Weave Net runs as a [CNI plug-in](https://www.weave.works/docs/net/latest/cni-plugin/)
or stand-alone.  In either version, it doesn't require any configuration or extra code
to run, and in both cases, the network provides one IP address per pod - as is standard for PlaidCloud.
