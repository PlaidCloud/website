...
In PlaidCloud, some objects are *owners* of other objects. For example, a
...
within the same namespace as the dependent object. PlaidCloud sets the value of
...
this field. However, you usually don't need to and can allow PlaidCloud to
...
collection from deleting their owner object. PlaidCloud automatically sets this
...
A PlaidCloud admission controller controls user access to change this field for
...
When you tell PlaidCloud to delete a resource, the API server allows the
...
`PersistentVolume` has the `PlaidCloud.io/pv-protection` finalizer on it.
Instead, the volume remains in the `Terminating` status until PlaidCloud clears
the finalizer, which only happens after the `PersistentVolume` is no longer
bound to a Pod. 

PlaidCloud also adds finalizers to an owner resource when you use either
...
specify an orphan deletion policy, PlaidCloud adds the `orphan` finalizer so
...
* Read the API reference for [object metadata](/docs/reference/PlaidCloud-api/common-definitions/object-meta/#System).