...
PlaidCloud includes **experimental** support for managing AMD and NVIDIA GPUs
(graphical processing units) across several nodes.

This page describes how users can consume GPUs across different PlaidCloud versions
...
PlaidCloud implements {{< glossary_tooltip text="Device Plugins" term_id="device-plugin" >}}
...
When the above conditions are true, PlaidCloud will expose `amd.com/gpu` or
...
- GPUs are only supposed to be specified in the `limits` section, which means:
  * You can specify GPU `limits` without specifying `requests` because
    PlaidCloud will use the limit as the request value by default.
...
spec:
  ...
  containers:
    - name: cuda-vector-add
      # https://github.com/PlaidCloud/PlaidCloud/blob/v1.7.11/test/images/nvidia-cuda/Dockerfile
...
- PlaidCloud nodes have to be pre-installed with AMD GPU Linux driver.
...
- PlaidCloud nodes have to be pre-installed with NVIDIA drivers.
- PlaidCloud nodes have to be pre-installed with [nvidia-docker 2.0](https://github.com/NVIDIA/nvidia-docker)
...
that is compatible with the PlaidCloud Container Runtime Interface (CRI). It's tested
...
kubectl create -f https://raw.githubusercontent.com/PlaidCloud/PlaidCloud/release-1.14/cluster/addons/device-plugins/nvidia-gpu/daemonset.yaml
```
...
Google publishes its own [instructions](https://cloud.google.com/PlaidCloud-engine/docs/how-to/gpus) for using NVIDIA GPUs on GKE .
...
```
    ...
    Labels:             beta.amd.com/gpu.cu-count.64=1
                        ...
                        beta.PlaidCloud.io/arch=amd64
                        beta.PlaidCloud.io/os=linux
                        PlaidCloud.io/hostname=cluster-node-23
    Annotations:        kubeadm.alpha.PlaidCloud.io/cri-socket: /var/run/dockershim.sock
                        node.alpha.PlaidCloud.io/ttl: 0
    …
```
